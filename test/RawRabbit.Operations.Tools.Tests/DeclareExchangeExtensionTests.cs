...
{
	public class DeclareExchangeExtensionTests
	{
		[Fact]
		public async Task Should_Invoke_InvokeAsync_With_Declaration_Overload()
		{
			...
			await mockBus.Object.DeclareExchangeAsync(declaration);
		...
		[Fact]
		public async Task Should_Add_ExchangeDeclaration_To_Context()
		{
			var declaration = new ExchangeDeclaration { Name = "test-exchange" };
		...
		public async Task Should_Invoke_InvokeAsync_With_Typed_Overload()
		{
			...
			await mockBus.Object.DeclareExchangeAsync<string>();
		...
		public async Task Should_Add_MessageType_To_Context()
		{
			...
			await mockBus.Object.DeclareExchangeAsync<int>();
		...
		{
			var declaration = new ExchangeDeclaration { Name = "test" };
			var mockBus = new Mock<IBusClient>();
			mockBus.Setup(b => b.InvokeAsync(It.IsAny<Action<IPipeBuilder>>(), It.IsAny<Action<IPipeContext>>(), It.IsAny<CancellationToken>()))
			...
			await Assert.ThrowsAnyAsync<OperationCanceledException>(() => mockBus.Object.DeclareExchangeAsync(declaration, cts.Token));
		}
	}
}
