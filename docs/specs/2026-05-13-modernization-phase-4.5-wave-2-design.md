...
| 7 | Logging | `src/RawRabbit/Logging/LibLog.cs` (1 file, 73 lines, Phase 3 modernization shim) | `test/RawRabbit.Tests/Logging/` (NEW subdir) | `LogProvider.For<T>()` returns wrapped logger; `LogProvider.LoggerFactory` setter accepts factory and propagates; `ILog : ILogger` conformance; 24 public extension methods on `ILog` (6 log levels × 4 overload shapes: msg, exception+msg, msg+exception with `*Exception` suffix; plus 6 `IsXxxEnabled` checks) each verified to dispatch to the correct underlying `ILogger` method per F1 strict-per-method resolution |
...
| 7 | Logging (LibLog: `LogProvider.For<T>()` + LoggerFactory setter + `ILog` conformance + 24 public ILog extension methods) | ~26 | ~26-50 | 0 (per F1 (a) — no consolidation) |
| 8 | Pipe (`PipeBuilder` + `PipeBuilderFactory` + ~19 IPipeContext extension methods) | ~5-9 | ~10-18 | 1 (parametric extension coverage) |
| | **Total** | **~61-91** | **~88-168** | **~3-9** |

**Acceptance gate: ≥88 net-new passing tests added by Wave 2 across the 8 sub-areas.**
...
1. **Test count:** ≥88 net-new passing tests across the 8 sub-areas (lower bound from §2).
...
4. **`RawRabbit.Tests` aggregate:** `dotnet test test/RawRabbit.Tests --no-build` returns ≥123 passed (was 41 post-Wave-1; +≥82 from Wave 2 areas 1, 2, 4, 5, 6, 7, 8 = 6+6+12+16+6+26+10). Skipped count remains 7.
...
| DW6 | Logging area kept in Wave 2 with strict-per-method count target (~26-50 tests) per CDR R1 F1 = (a) | Verification surfaced: `Logging/` is just `LibLog.cs` (Phase 3 shim) — but exposes 24 public `ILog` extension methods (6 log levels × 4 overload shapes) in addition to `LogProvider` + `ILog` conformance. F1 resolution = (a) strict per-method, no consolidation: each of the 24 forwarding extensions gets ≥1 happy + ≥1 error test (typically: mock `ILogger`, call extension, verify dispatch). Convention applies to Waves 3-5 (their forwarding extensions also tested individually) | Drop Logging from Wave 2 entirely (rejected — public surface); F1 (b) extend exemption to forwarding methods (rejected per user pick); F1 (c) mandate `[Theory]` consolidation (rejected per user pick) |
...
| F1 | CDR R1 §3 F1: how to apply per-method strict to 1-line forwarding extension methods (24 in LibLog; pattern recurs in `Operations.*` + `Enrichers.*` extensions in Waves 3-5) | User decision (CDR R1 follow-up): **(a) strict per-method, no consolidation, no exemption.** Each forwarding extension gets ≥1 happy + ≥1 error test individually. Convention applies to Waves 3-5 (their forwarding extensions also tested individually). Wave 2 area 7 lower bound rises from ~2-4 to ~26-50; wave-wide gate from ≥64 to ≥88; RawRabbit.Tests aggregate gate from ≥99 to ≥123 |
...
| A5 | `src/RawRabbit/Logging/` exists with testable adapter/wiring source files | `ls src/RawRabbit/Logging/*.cs` returns 1 file: `LibLog.cs` (73 lines, Phase 3 modernization shim). Public surface: `LogProvider` (`For<T>()` + `LoggerFactory` setter), `ILog : ILogger`, `LogWrapper internal sealed`, AND 24 public extension methods on `ILog` in a static extension class (6 log levels × 4 overload shapes including `IsXxxEnabled` checks). Smaller-than-parent-spec scoping noted in FD2; full extension-method surface enumerated post-CDR-R1 per F1 resolution |
