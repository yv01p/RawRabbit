...
with the same warning shape as Phase 1 (NU1701/NU1902/NU1903 only — no new errors). Test count unchanged from Phase 1's 30 passing / 9 skipped / 0 failed. `dotnet pack` produces 25 `.nupkg` files for the library projects under `src/`. Tests and samples produce no `.nupkg` (they are non-packable). No publish.
...
| 4 | `dotnet pack` finds a problem we didn't have to worry about under just `dotnet build` | First time the repo runs `dotnet pack` post-Phase-1; Phase 1 only validated build/test | If pack fails: read the actual error; either fold the fix into Phase 2 (if metadata-related) or escalate to a spec-amendment. Do NOT add cosmetic metadata to silence pack warnings — those are spec §2.3 deferred items. |
