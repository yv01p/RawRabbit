...
**Operations projects** (8 areas): per project, per-middleware happy + error path; per public extension method on `IBusClient`; per Builder/Stage/Key type if present. Mock at `IChannel` / `IPipeContext` boundary. Verify pipe chain composition without touching a real broker. Estimated 8–15 tests per project → ~80–120 total.

**Enrichers** (7 areas): per enricher, per-middleware context manipulation correct; side effects observable on the next pipe context; configuration override paths work; missing config falls back to documented default. Polly extends existing 1-pass + 2-skip. Estimated 4–12 tests per enricher → ~40–60 total.
...
| D3 | Mock at the broker boundary (`IChannel`, `IConnection`) using Moq 4.20 | Matches existing pattern; the 7 currently-skipped Channel tests confirm Moq has known issues at this boundary on net10 — those tests stay skipped (Phase 5/7) but new tests in less-fragile areas use the same proven pattern | Mock at higher `IBusClient`/`IPipeContext` boundary (more setup per test, may miss low-level integration bugs); per-project decision (more design overhead); defer all broker-touching coverage to Phase 6 (would shrink Phase 4.5 to a fraction of "comprehensive") |
