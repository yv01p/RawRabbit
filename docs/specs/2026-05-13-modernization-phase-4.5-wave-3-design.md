...
7. **Skipped count:** aggregate skipped = 9 + N (where N = Wave-3-newly-blocked broker-mock signature tests, per parent R2). Each new skip carries the per-test "Phase 5/7 territory: <specific signature>" annotation matching the existing 7 skips' format. N expected = 0-5; if N>0, the post-Wave-3 skipped baseline becomes (8 + N) in `test/RawRabbit.Tests` + 1 in `ServiceCollection.Tests`.
...
| A13 | Post-Wave-2 baseline = 379 passed / 7 skipped in `RawRabbit.Tests`; 21 / 1 in `ServiceCollection.Tests` | Verified at session start: `dotnet test test/RawRabbit.Tests --no-build -c Release` returned `Failed: 0, Passed: 379, Skipped: 7`; `dotnet test test/RawRabbit.DependencyInjection.ServiceCollection.Tests --no-build -c Release` returned `Failed: 0, Passed: 21, Skipped: 1`. §4 gate 4 + gate 5 anchored on these numbers |
